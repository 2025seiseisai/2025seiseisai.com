...
export default function Hamburger({ open, setOpen }: { open: boolean; setOpen: (open: boolean) => void }) {
    ...
    return (
        <button
            className={`z-100000002 flex aspect-1/1 h-[30px] w-auto cursor-pointer items-center justify-center
                transition-all duration-500 outline-none md:h-[40px]
                ${open ? styles.opening : "mr-[min(35px,7svw)] md:mr-[min(40px,8svw)]"}`}
            onClick={() => {
                setTimeout(() => {
                    setOpen(!open);
                }, 30);
