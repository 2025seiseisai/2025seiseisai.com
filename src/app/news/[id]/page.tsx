...
export default async function Page({ params }: { params: Promise<{ id: string }> }) {
    ...
    const mdx = await compileMDX({
        ...
        components: {
            ...
            h1: ({ children }: { children: any }) => {
                return (
                    <h1 className="mt-[8px] mb-[8px] border-l-4 border-[#de0d22] pl-[16px] text-[28px] not-md:border-l-3 not-md:pl-[10px] not-md:text-[24px]">
            ...
            h2: ({ children }: { children: any }) => {
                return <h2 className="mt-[8px] mb-[8px] text-[26px] not-md:text-[22px]">{getJSX(getJSX, children)}</h2>;
            },
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            h3: ({ children }: { children: any }) => {
                return <h3 className="mt-[6px] mb-[6px] text-[24px] not-md:text-[21px]">{getJSX(getJSX, children)}</h3>;
            },
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            h4: ({ children }: { children: any }) => {
                return <h4 className="mt-[5px] mb-[5px] text-[22px] not-md:text-[20px]">{getJSX(getJSX, children)}</h4>;
            },
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            h5: ({ children }: { children: any }) => {
                return <h5 className="mt-[5px] mb-[5px] text-[21px] not-md:text-[19px]">{getJSX(getJSX, children)}</h5>;
            },
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            h6: ({ children }: { children: any }) => {
                return <h6 className="mt-[5px] mb-[5px] text-[20px] not-md:text-[18px]">{getJSX(getJSX, children)}</h6>;
            },
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    return (
        <>
            <div className="lg: lg:mr-[20%] lg:ml-[20%] lg:items-center mr-[20px] mb-[60px] ml-[20px] flex flex-col text-[#0b0e0f] not-md:mb-[40px] md:mr-[10%] md:ml-[10%]">
                <div className="mt-[30px] flex items-center border-b-[2px] border-[#0b0e0f] pb-[4px] md:pb-[8px] lg:w-[120%] lg:justify-center lg:pb-[12px]">
                ...
                <div className="mt-[24px] flex items-center not-md:mt-[20px]">
                    ...
                    {importance && (
                        <div className="ml-[12px] flex h-[24px] items-center justify-center rounded-full bg-[#de0d22] pr-[12px] pl-[12px] text-[13px] text-white not-md:ml-[8px] not-md:h-[20px] not-md:pr-[10px] not-md:pl-[10px] not-md:text-[12px]">
                            重要
                        </div>
                    )}
                </div>
