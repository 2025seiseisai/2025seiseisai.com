            ...
            <div className={"mt-[24px] mb-[60px] not-md:mt-[16px] not-md:mb-[40px]"}>
                {news.map(({ id, date, importance, title, link }, idx) => {
                    ...
                    return (
                        <div
                            key={id}
                            className={`${onlyImportant && !importance ? "max-h-[0px]" : "max-h-[60px] not-md:max-h-[50px]"}
                            ...
                            <Link
                                className={`${
                                    idx <= firstIndex || (onlyImportant && !importance)
                                        ? "border-[#de0d2200]"
                                        : "border-[#de0d22]"
                                } relative flex h-[60px] items-center border-t-[1.5px] text-[16px]
                                transition-[border-color] duration-250 ease-out not-md:h-[50px]`}
                                href={`/news/${link}`}
                            >
                                {importance && (
                                    <div
                                        className={`absolute top-0 h-[15px] w-[15px] bg-[#de0d22]
                                        [clip-path:polygon(0_0,100%_0,0_100%)] not-md:h-[12px] not-md:w-[12px]`}
                                    ></div>
                                )}
                                <h3
                                    className={`mr-[8.33svw] ml-[calc(2.78svw_+_15px)] not-md:mr-[6svw]
                                    not-md:ml-[calc(2.78svw_+_12px)] not-md:text-[13px]`}
                                >
                                    {year}.{month}.{day}
                                </h3>
