...
function DownloadButton({ url, filename, filesize }: { url: string; filename: string; filesize: string }) {
    return (
        <Link
            download
            href={url}
            className={`mx-auto mt-[20px] flex h-12.5 w-max max-w-19/20 items-center rounded-[12px] border-3
                border-[#dedede] bg-[#fefefe] text-sm/normal text-pri-black transition-[box-shadow] duration-200
                hover:[box-shadow:_#0b0e0f20_0px_0px_5px] b:h-15 b:max-w-4/5 b:text-lg/normal`}
        >
...
export default async function Page({ params }: { params: Promise<{ round: string; index: string }> }) {
    ...
    return (
        <>
            ...
            <h1
                className={`mx-auto mt-[25px] max-w-[90svw] border-b-2 border-pri-black text-start text-[1.75rem]/normal
                    font-medium text-pri-red b:mt-[30px] b:px-[100px] b:text-center b:text-[2.5rem]`}
            >
                {title}
            </h1>
            <article>
                ...
                <div
                    className={`mt-[40px] mr-auto ml-auto w-[90dvw] b:mt-[80px] b:ml-[8dvw] b:w-[56dvw]
                        b:max-w-[calc(84dvw-275px)]`}
                >
                    <div>{description}</div>
