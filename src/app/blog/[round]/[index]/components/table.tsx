...
export default function Table({ toc }: { toc: { name: string; id: string }[] }) {
    ...
    return (
        <div className="rounded-[20px] border-3 border-[#dedede] p-[20px] text-[#0b0e0f]">
            <div className="mb-[5px] text-[19px]/normal font-medium">目次</div>
            <ul>
                {toc.map((item) => (
                    <a href={`#${item.id}`} key={item.id}>
                        <li
                            className={`mx-[5px] flex w-full text-base text-[13px] leading-[1.7] transition-all duration-300 before:content-['・'] hover:opacity-75 ${item.id === activeTitleId && "bg-[#de0d2221]"}`}
                        >
